...
However since the implementation was quite time-consuming, in this time first there was totally unrelated drama with the devs, then Qodot was radically rewritten (ditched C# in favor of GDScript). Since everything I wrote was in C#, plus the drama, this code is basically now dead, which is pretty sad. I would like to resume this project, but it would require the Qodot devs to come forward and resolve the trust issues created.
